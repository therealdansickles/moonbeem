...
export class Tier extends BaseEntity {
  ...
  @Column({ nullable: true, length: 500, comment: "This is the URL that will appear with the asset's image and allow users to leave the marketplace and view the item on your site." })
  externalUrl?: string;
  ...
  // https://docs.bnbchain.org/docs/nft-metadata-standard/
  @Column({ nullable: true, length: 500, comment: "A human readable description of the item. Markdown is supported." })
  description?: string;

  @Column({ nullable: true, type: "jsonb", comment: "A JSON object with arbitrary data. This can be used to store any additional information about the item." })
  attributes?: string;

  @Column({ nullable: true, length: 6, comment: "Background color of the item. Must be a six-character hexadecimal without a pre-pended #." })
